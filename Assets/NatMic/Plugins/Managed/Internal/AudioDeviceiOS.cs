...
namespace NatMic.Internal {
    ...
    public sealed class AudioDeviceiOS : AudioDevice {
        ...
        public override bool EchoCancellation {
            get => device.DeviceEchoCancellation();
        }
        #endregion
        ...
        public override bool StartRecording (int sampleRate, int channelCount, IAudioProcessor processor) {
            ...
            return device.StartRecording(sampleRate, channelCount, OnSampleBuffer, (IntPtr)self);
        }
        ...
        private AudioDeviceiOS (IntPtr device) => this.device = device;

        ~AudioDeviceiOS () => device.FreeDevice();

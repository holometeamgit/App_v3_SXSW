...
namespace Beem.UI {
    ...
    public class InfiniteScroll : MonoBehaviour, IDropHandler {
        ...
        float ContainerHeight {
            get {
                return _containerRect.height;
            }
        }

        float ContainerWidth {
            get {
                return _containerRect.width;
            }
        }
        ...
        public void InitData(int itemsCount) {
            HelperFunctions.DevLog("InitData itemsCount " + itemsCount + " containerRectTransform " + containerRectTransform);
            _containerRect = containerRectTransform.rect;
            if (Type == 0) {
                InitVertical(itemsCount);
            } else {
                InitHorizontal(itemsCount);
            }
        }
        ...
        void Awake() {
            ...
            containerRectTransform.ForceUpdateRectTransforms();
        ...
        void CreateViewsHorizontal() {
            ...
            int fillCount = Mathf.RoundToInt(ContainerWidth / width) + 4;
            _views = new GameObject[fillCount];
            for (int i = 0; i < fillCount; i++) {
                clone = (GameObject)Instantiate(Prefab, Vector3.zero, Quaternion.identity);
                clone.transform.SetParent(_content);

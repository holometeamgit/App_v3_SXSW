...
public class ThumbnailWebDownloadManager : MonoBehaviour {
    ...
    private string GetRequestStreamURL(ThumbnailWebRequestStruct thumbnailWebRequestStruct) {
        ...
        var builder = new UriBuilder(_webRequestHandler.ServerURLMediaAPI + videoUploader.Stream);
        builder.Port = -1;
    ...
    private string GetRequestStreamByIdURL(long id) {
        return _webRequestHandler.ServerURLMediaAPI + videoUploader.StreamById.Replace("{id}", id.ToString());
    }

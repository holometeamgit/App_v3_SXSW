...
public class DeepLinkRoomController : MonoBehaviour {
    ...
    private void GetRoomLink(string source) {
        if (!serverURLAPIScriptableObject.UseHashForRoomLink) {
            ...
            DynamicLinkParameters dynamicLinkParameters = new DynamicLinkParameters(serverURLAPIScriptableObject.FirebaseDynamicLink, serverURLAPIScriptableObject.ARUrl, DynamicLinkParameters.Parameter.room, source, SocialParameters(source));

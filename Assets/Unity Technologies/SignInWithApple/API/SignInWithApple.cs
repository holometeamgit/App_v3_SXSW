    ...
    public class SignInWithApple : MonoBehaviour
    {
        ...
        private static void LoginCompletedCallback(int result, [MarshalAs(UnmanagedType.Struct)]UserInfo info)
        {
            ...
            if (result != 0)
            {
                ...
                Debug.Log("idToken " + args.userInfo.idToken);
                Debug.Log("displayName " + args.userInfo.displayName);
                Debug.Log("email " + args.userInfo.email);
                Debug.Log("userId " + args.userInfo.userId);
                Debug.Log("userDetectionStatus " + args.userInfo.userDetectionStatus);
            } else
            {
